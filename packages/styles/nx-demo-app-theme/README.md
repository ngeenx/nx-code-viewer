...
```css
@reference '@ngeenx/nx-demo-app-theme';
```

## Symlink Setup

This library includes a `symlink` target that creates a symlink in `node_modules/@ngeenx/nx-demo-app-theme` pointing to the source CSS file. This enables proper resolution of `@reference` imports.

### Running the symlink target manually

```bash
nx run nx-demo-app-theme:symlink
```
...
```json
{
  "targets": {
    "serve": {
      ...
      "dependsOn": ["nx-demo-app-theme:symlink"]
      // ... rest of config
    }
  }
}
