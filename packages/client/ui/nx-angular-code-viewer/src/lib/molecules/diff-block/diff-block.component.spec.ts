...
describe('DiffBlockComponent', () => {
  ...
  function create(
    ...
    if (opts.viewMode !== undefined)
      fixture.componentRef.setInput('viewMode', opts.viewMode);
  ...
  describe('theme', () => {
    it('applies dark class to the container', () => {
      ...
      expect(
        nativeElement
          .querySelector('.diff-block-container')
  ...
  describe('unified view', () => {
    ...
    it('renders the hunk header', () => {
      const { nativeElement } = create({
        hunks: [makeHunk({ header: '@@ -1,2 +1,2 @@' })],
      });
      expect(nativeElement.querySelector('.hunk-header').textContent).toContain(
        '@@ -1,2 +1,2 @@'
      );
    });
    ...
    it('renders diff-line components for each line', () => {
      const { nativeElement } = create();
      expect(
        nativeElement.querySelectorAll('nx-diff-line').length
      ).toBeGreaterThan(0);
    });
  ...
  describe('collapsedRangeToggle output', () => {
    it('emits when a diff-collapsed-indicator fires toggle', () => {
      ...
      fixture.componentInstance.collapsedRangeToggle.subscribe(
        (r: DiffCollapsedRange) => emitted.push(r)
      );
      ...
      fixture.debugElement
        .query(By.css('.diff-block-container'))
