 ...
 * Mirrors Angular's providedIn: 'root' singleton pattern
 */
const copyStates = new Map<string, Ref<CopyButtonState>>();
const stateTimeouts = new Map<string, ReturnType<typeof setTimeout>>();

/**
 * Composable for clipboard operations with reactive state management
 * Provides multi-instance state tracking via unique keys
 */
export function useClipboard() {
  function getCopyState(key: string): Ref<CopyButtonState> {
    let state = copyStates.get(key);

    if (!state) {
      state = ref<CopyButtonState>('idle');
  ...
  function isCopied(key: string) {
    const state = getCopyState(key);
    return computed(() => state.value === 'copied');
  }
  ...
  function updateCopyState(key: string, state: CopyButtonState): void {
    const stateRef = getCopyState(key);
