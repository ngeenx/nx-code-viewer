...
function resolveLink(template: string, captureGroups: string[]): string {
  let result = template;
...
function extractLines(html: string): string[] {
  ...
  if (lines.length === 0 && html.trim()) lines.push(html);
  return lines;
}
...
function createProcessedReference(
  ...
  if (types.includes('link') && config.link) {
    resolvedLink = sanitizeUrl(resolveLink(config.link, captureGroups));
  }
...
function processTokenWithMatches(
  ...
  const wrappedContent = wrapTextWithReferences(
    token.text, token.textStart, matches, startMatchIndex, matchToProcessed, encodeHtmlEntities
  );
...
function injectReferences(
  ...
  for (let i = 0; i < matches.length; i++) {
    matchToProcessed.set(i, createProcessedReference(matches[i], lineNumber, processedReferences));
  }
  ...
  for (const token of tokens) {
    ...
    result.push(processTokenWithMatches(token, matches, matchIndex, matchToProcessed));
...
export function processReferences(
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    processedLines.push(processLine(lines[lineIndex], references, lineIndex + 1, processedReferences));
  }

  const resultHtml = processedLines.map(line => `<span class="line">${line}</span>`).join('');
